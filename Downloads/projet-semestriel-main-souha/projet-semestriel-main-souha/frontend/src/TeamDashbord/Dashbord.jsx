...
function CommitsTable({ members }) {
  ...
  const commits = useMemo(() => {
    return members.flatMap(m =>
      (m.history || []).flatMap(h =>
        (h.commitMessages || []).map(msg => ({
          author: m.author,
          date: h.date,
          linesAdded: Math.round(h.linesAdded / (h.commitMessages?.length || 1)),
          linesDeleted: Math.round(h.linesDeleted / (h.commitMessages?.length || 1)),
          score: h.dailyScore,
          message: msg
        }))
      )
  ...
  return (
    <div className="commits-box">
      ...
      <div className="commits-foot">
        <span>{commits.length} commits au total</span>
        {commits.length > 5 && (
          <span className="voir-plus" onClick={() => navigate("/all-commits", { state: { members } })}>
...
function ActivitySummary({ member }) {
  ...
  const refactorIndex = member.linesAdded > 0 
    ? Math.round((member.linesDeleted / member.linesAdded) * 100) 
    : 0;

  const density = member.commitCount > 0 
    ? Math.round(member.linesAdded / member.commitCount) 
    : 0;

  const diversity = member.commitCount > 0 
    ? (member.filesModified / member.commitCount).toFixed(1) 
    : 0;

  return (
    <div className="act-box">
      <div className="act-title">ANALYSE QUALITATIVE</div>
      <div className="act-grid">
        <div className="act-card">
          <div className="act-ico" style={{ background: "rgba(52,211,153,.12)" }}>🧹</div>
          <div className="act-val">{refactorIndex}%</div>
          <div className="act-lbl">Indice Refactor<br/><span>(Code supprimé)</span></div>
        </div>
        <div className="act-card">
          <div className="act-ico" style={{ background: "rgba(129,140,248,.12)" }}>💎</div>
          <div className="act-val">{density}</div>
          <div className="act-lbl">Lignes / Commit<br/><span>(Densité moyenne)</span></div>
        </div>
        <div className="act-card">
          <div className="act-ico" style={{ background: "rgba(251,191,36,.12)" }}>🌿</div>
          <div className="act-val">{diversity}</div>
          <div className="act-lbl">Fichiers / Commit<br/><span>(Impact moyen)</span></div>
        </div>
      </div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const [syncing, setSyncing] = useState(false); // État pour le bouton actualiser
  ...
  // 🔄 FONCTION ACTUALISER (Synchronisation Git)
  const handleSync = async () => {
    const path = localStorage.getItem("current_repo_path");
    if (!path) return alert("Veuillez d'abord effectuer une recherche pour définir le projet.");

    setSyncing(true);
    try {
      const response = await fetch(`http://localhost:8080/api/performance/analyze-local?path=${encodeURIComponent(path)}`, {
        method: "POST"
      });
      if (response.ok) {
        fetchStats(); // Recharge les données après l'analyse
      }
    } catch (err) {
  ...
  return (
    <div className="app">
      <div className="topbar">
        ...
        {/* 🟢 BOUTON ACTUALISER DANS LA TOPBAR */}
        <button 
          onClick={handleSync} 
          disabled={syncing}
          style={{
            marginLeft: 'auto',
            background: syncing ? '#1e2240' : '#3b6ef6',
            color: 'white',
            border: 'none',
            padding: '8px 16px',
            borderRadius: '8px',
        ...
        >
          {syncing ? "🔄 Analyse..." : "↻ Actualiser Git"}
        </button>
      </div>
