...
export default function Search() {
  ...
  const isValidInput = (val) =>
    val.trim().startsWith("http") || val.trim().includes(":/") || val.trim().includes("\\") || val.trim().includes(".git");
  ...
  const handleSearch = async (searchUrl) => {
    ...
    localStorage.setItem("current_repo_path", target.trim()); // 🟢 AJOUTÉ
    navigate("/dashboard");
} else {
      ...
      <div className="gs-card">
          ...
          {showHistory && history.length > 0 && (
            <div style={{
              ...
              <div style={{
                ...
                <span onClick={clearHistory} style={{ fontSize: "10px", color: "#f87171", cursor: "pointer", fontWeight: 600 }}>
                  Tout effacer
                </span>
              </div>
              ...
              {history.map((h, i) => (
                ...
                >
                  ...
                  <span onClick={(e) => removeFromHistory(h, e)} style={{
                    color: "#4a6582", fontSize: "14px", cursor: "pointer", flexShrink: 0, fontWeight: 700, lineHeight: 1
                  }}>×</span>
                </div>
              ))}
            </div>
        ...
        {!showHistory && history.length > 0 && (
          <div style={{ marginTop: "20px" }}>
            <div style={{ fontSize: "10px", color: "#4a6582", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", marginBottom: "10px" }}>
              Recherches récentes
            </div>
            {history.slice(0, 5).map((h, i) => (
              ...
              >
                ...
                <span onClick={(e) => removeFromHistory(h, e)} style={{ color: "#4a6582", fontSize: "16px", cursor: "pointer", fontWeight: 700 }}>×</span>
              </div>
            ))}
          </div>
        )}
